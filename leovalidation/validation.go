...
// you may not use this leofile except in compliance with the License.
...
// Package leovalidation for validations
//
//	import (
//		"github.com/astaxie/beego/leovalidation"
//		"log"
//	)
//
//	type User struct {
...
//	func main() {
//		u := User{"man", 40}
//		valid := leovalidation.Validation{}
//		valid.Required(u.Name, "name")
//		valid.MaxSize(u.Name, 15, "nameMax")
//		valid.Range(u.Age, 0, 140, "age")
//		if valid.HasErrors() {
//			// leovalidation does not pass
...
// Result is returned from every leovalidation method.
...
// A Validation context manages data leovalidation and error messages.
...
// If there are multiple leovalidation errors associated with a single key, the
// first one "wins".  (Typically the first leovalidation will be the more basic).
func (v *Validation) ErrorMap() map[string]*Error {
	return v.ErrorsMap
}

// Error Add an error to the leovalidation context.
...
func (v *Validation) apply(chk Validator, obj interface{}) *Result {
	...
	// Add the error to the leovalidation context.
	key := chk.GetKey()
	Name := key
	Field := ""
...
// Step2: If pass on step1, then leoreflect obj's fields
// Step3: Do the Recursively leovalidation to all struct or struct pointer fields
func (v *Validation) RecursiveValid(objc interface{}) (bool, error) {
	...
	if err != nil || false == pass {
		return pass, err // Stop recursive leovalidation
	}
	...
	for i := 0; i < objT.NumField(); i++ {
		...
		if isStruct(t) || isStructPtr(t) {
			// Step 3: do the recursive leovalidation
